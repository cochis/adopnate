...
export class SeoService {
  ...
  createLinkForCanonicalURL() {
    ...
    const linkAlternate: HTMLLinkElement = this.doc.createElement('link');
  ...
  generateTags(config: any) {
    config = {
      ...
      keywords:
      'Donar, adoptar, mascota , integrate de familia , amigo , compañero ',
      image: '',
      slug: '',
      colorBar: '#3f3697',
      ...config,
    ...
    this.meta.updateTag({
      property: 'og:url',
      content: `https://adopnate-3a16c.web.app/${config.slug}`,
    });
  }
}
